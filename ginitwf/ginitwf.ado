...
program define ginitwf, rclass
    ...
    * Manejo especial de la expresión de peso
    if "`weight'" != "" {
        local wexp `"`exp'"'
        local peso_exp = subinstr(`"`wexp'"', "=", "", 1)
        ...
        * Verificar que la expresión de peso sea válida
        capture confirm numeric variable `peso_exp'
        if _rc {
            di as error "La variable de peso '`peso_exp'' debe ser numérica"
            exit 111
        }
    }
    ...
    * Verificar variable de agrupación si se especificó
    if "`by'" != "" {
    ...
    * Verificar variables levels y xi si se especificaron
    if "`levels'" != "" {
        capture confirm variable `levels'
        if _rc {
            di as error "La variable de niveles '`levels'' no existe en el dataset"
            exit 111
        }
    }
    
    if "`xivar'" != "" {
        capture confirm variable `xivar'
        if _rc {
            di as error "La variable xi '`xivar'' no existe en el dataset"
            exit 111
        }
    }
...
program define ginitwfhelp
    version 14
    di _n
    di as text "GINITWF - VERSIÓN 3.1.0 (COMPATIBLE)"
    di as text "=================================="
    di _n
    ...
    di _n
    di as text "IMPORTANTE: Las variables de peso deben existir antes de usar ginitwf"
    di as result "     . gen peso = 1  // Crear variable de peso constante"
    di _n
    ...
    di as text "VERSIÓN: 3.1.0 (30 Marzo 2025)"
end
